...
public class GameModel {
    ...
    public GameModel(String player1Name, Color player1Color, String player2Name, Color player2Color,  int horizontalSize, int verticalSize) throws IllegalArgumentException {
        ...
        if (player1Color.equals(Block.SAFE_COLOR) || player2Color.equals(Tile.SAFE_COLOR)) {
        ...
        // initialize game map
        this.map = new Block[verticalSize][horizontalSize];
        ...
        for (int row = 0; row < verticalSize; row++) {
            for (int column = 0; column < horizontalSize; column++) {
                ...
                if (row == 0 || column == 0 || column == horizontalSize - 1 || row == verticalSize - 1 ) {
                    this.map[row][column] = new Block(Color.BLACK);
                } else {
                    this.map[row][column] = new Block();
                }
            }
        }
    ...
    public GameState doRound() throws SQLException {
        for (int i = 0; i < this.players.length; ++i) {
            ...
            Block playerLocation = this.map[player.getVerticalLocation()][player.getHorizontalLocation()];
            ...
            if (!playerLocation.isSafe()) {
                if (i == 0) {
                    DBRepo.instance().insertScore(players[1].getName());
                    return GameState.PLAYER2WON;
                } else {
                    DBRepo.instance().insertScore(players[0].getName());
